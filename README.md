...
Regarding RQ2, we examined different approaches to address our research questions by analyzing three examples from distinct fields: (1) open-source projects, (2) nutrient profiling, and (3) quality-of-life indices. While rating systems exist in other fields, we chose these three because they are well-studied in the research community and have practical applications in everyday life. They also address aspects that vary across software engineering, science, and social impact.
...
A variety of nutrient profile models have been developed by academics, health organizations, national governments and the food industry. The development or selection of a model to use in food policy decisions is important, as different models can lead to different classifications of the same foods. some of these methods grounds nutritional score systems like the Nutri-Score adopted in many European countries.

For example, the British and French food safety agencies developed the WXYfm and SAIN,LIM models, respectively. Although their predictive validity in relation to chronic disease has yet to be demonstrated, they constitute valid examples about the design of classification methods.

**WXYfm model.** The WXYfm model was developed for the Food Standards Agency (FSA) for the regulation of food advertising on television programs aimed at children (Rayner et al., 2005a). The model first allocates points on the basis of the nutritional content per 100g of the food or drink. Foods and drinks are then classified into healthiness categories.

**Nutri-Score.** The Nutri-Score is a five-colour nutrition label that aims to guide consumers in the buying of food and cosmetics products by assigning products a rating letter from A (best) to E (worst), with associated colors from green to red. The system is based on the Nutri-score system, which gives a score to products based on the model WXYfm,[^arambepola] as a result of research work from academia. Then, a calculation on this set of point, compute the total score for the product. Based on the total score, a label ranging from A (best) to E (worst) is assigned. Some applications have tweaked this score system to calculate what they claim to be a better result. Divergent visions from governments have raise concerns about the validity of these apps. So, the nutritional value deeply relies on the achievement from the model WXYfm and the consequent scorign system, which awards 0 to 10 points for energy value and ingredients that should be limited in the diet. The final calculation is a direct consequence of this evaluation. Since 2022, the Scientific Committee of the Nutri-Score recommends some changes in the algorithms which mostly impacts the point system. The total score is then rendered as a five-color label that guides consumers in the correct choice of products.

The app grounds its computation on the Nutri-score system, a five-color nutritional rating system that attempts to simplify the nutritional rating system demonstrating the overall nutritional value of food products. The team of developers from Yuka, have later tweaked the Nutri-score to better fit their needs, and results are then displayed through graphics and symbols to communicate the nutritional values to the target audience.[^yuka_rating] However, the Nutri-score is not the only example of nutritional rating system and the Yuka app is not the only rating app for food and cosmeticas. Other rating systems have been developed by governments, non-profit organizations, private institutions, and companies. They mostly differentiate themselves from the database they rely on, the process to compute the ranking, and the way to showacse results. Common methods include point systems to rate foods based on general nutritional value or ratings for specific food attributes, such as cholesterol content. The following is a non-comprehensive list of methods to rate food.
