...
This work is inspired by past exercises that define SSI principles[^allen][^ferdous][^toip][^toth]. However, we depart from previous contributions because the scope of our analysis is to use these principles for developing a model to assess identity solutions based on the promise of user control over information. While this is not the first contribution aimed at assessing the quality of identity systems, other SSI-based assessments have either failed to describe their methodology[^pava], lacked extensive precision in their analysis[^bokkem], or did not explain how they derived the criteria for their system analysis[^omar]. To avoid similar shortcomings, our exercise is fully replicable, as we employed a quantitative method to select properties and a clustering technique to categorize them. The quantitative analysis is then complemented by a qualitative analysis through a survey with experts participating in the ARES conference 2024 to refine our results.

At the same time, as we struggled to find assessment models in the digital identity sphere, with the exception of a few works, we examined how evaluation systems have been designed in other fields of study by analyzing three different examples from (1) open-source projects, (2) nutrient profiling scales, and (3) the Where-to-be-born Index (formerly the Country Quality-of-Life Index). Although they are not directly relevant to digital identity, and other rating systems exist in other fields, such as University Ranking and cost of living, they deal with a mix of qualitative and quantitative data. Indeed, they include intangible aspects such as the political impacts of software, cultural aspects of food and products, individual freedom, and happiness. Hence, our purpose, beyond the sphere of identity, is to understand the rationale that grounds some of the decisions made by designers when creating a rating model. At the same time, we aim to understand if a rule of thumb exists for the definition of evaluation models.

*1. FOSS projects evaluation*
FOSS (Free and Open Source Software) projects are software initiatives where the source code is made publicly available for anyone to view, use, modify, and distribute.[^fortunato] Over the years, FOSS projects became synonims for projects that prioritize transparency, collaboration, and community-driven development. Notable examples of FOSS projects are Linux distributions, LibreOffice, GIMP, Blender, and VLC. Beyond these projects, numerous other software applications are freely distributed online but do not open their code to the community. Having an open source code may lead to a number of advantages, most and foremost the patch and bugs are fixed quickly.[^dehinbo] Users often rely on quick bug fixes to secure their software. It is then crucial to distinguish between projects that genuinely embrace the FOSS philosophy and those that do not. 

Past empirical studies define criteria for the evaluation of FOSS projects and solicits these criteria to forge specific ways to evaluate each project.[^foss] The OpenMRS makes use of these criteria to evaluate projects on GitHub.[^foss] The Project then uses a score-based rubric to evaluate Free and open source projects,[^foss_rubric]. In some cases, the appropriateness of a project is determined, for the most part, by the answers to a set of important questions.[^hunter] Some researchers have suggested that the questions should fall into three categories related to the project: viability, approachability, and suitability. The work in [^heidi] suggests to tag questions in two categories, such as critical and not critical or secondary. Follow up this idea, the work in [^evaluation_activity] defines principles and split them into two subclasses: mission critical criteria (primary) and secondary criteria.

Attempts to quantitatively evaluate the contribution of open source projects is proposed by the OpenBRR (Open Business Readiness Rating), as an evaluation method based on gathering metrics and factual data on ten categories, and in each category, a set of criteria and metrics are proposed. The first step in the process is to select and weight criteria to be use in the evaluation process. In the following step, each criteria is evaluated on its own. Inputs are then weighted and each category is given a rating ranging from 1 to 5. In order to obtain a total score for the functionality criteria, the total weights of the standard functionality items is summed up in W. Then, depending on the final usage the software, adopters may even weight the categories, thus obtaining the overall rating for the project. Then the score for the assessed tool is obtained by adding all the scores, both from the standard and extended functionality. As we may understand, not all categories are weighted equally, or for some scenarios a category may not be considered at all for the final rating.
