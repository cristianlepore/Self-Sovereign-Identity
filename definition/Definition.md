...
|----------|----------|----------|
| Existence ~~and Representation~~ | A Self-Sovereign Identity system should enable users to digitally encode their characteristics as attributes, which collectively define their identity. Instead of being a single, fixed entity, identity in this context is dynamic, allowing users to selectively combine different attributes based on specific needs. Depending on verifier requirements, users must be able to present varying sets of attributes, with each selected combination defining their identity in that particular context. | The emephasis is on identity as a set of attributes that users present to prove their existence. The moderate importance of having multiple identities has been claimed by [^ferdous][^cucko] and reinforced through the survey. In this context, representation of identity is self-included in the definition of existence, and therefore deleted from the label of the property that becomes just existence. |
| ~~Ownership and~~ Control | Users must have full control of their digital identity and corresponding attributes. This has led to the emergence of the Digital Wallet concept, which securely stores all identity-related personal and confidential information on the user’s device. The device must be owned and controlled by the user.  The users may delegate (he control of their identity to agents and guardians of their choice, including individuals, organizations, devices, and software. | The definition from authors have been condensed and the presence of digital facets emphasized from [^naik]. More than 50% of survey respondents reported at least a moderate level of importance for this principles, with the ToIP leveraging the possibility to delegate control to guardians,[^toip]. For this reason, ownerships was embedded within control. |
| Access ~~and Availability~~ | Users must have unrestricted access of their information and to the list of identity and service providers. They must be able to access and retrieve information without any discrimination based on their ethnicity, gender, socio-economic status, or language, and regardess their location. | The majority of respondents to the survey foreseen access to identity data from different locations as important or very important. Some of the authors [^naik][^glockler] advocate for a non-discriminarory access of information based on location and socio-economic aspects. For this reason, the definition touch both aspects and extends the concept of access to the list of identity and service providers. At the same time, any consideration about usability from [^toip][^sovrin] was taken out because out of scope. |
| Transparency | The identity system, algorithms, and protocols to manage identities should open-source and verifibale to understand rules and policy. Anyone should be able to examine how they work. | More than 50% of survey respondents considered transparency as important or very important to study the functioning of the system. The definition does not consider the independency of protocols.[^naik] |
| Persistence | Identity must persist as long as the users wish, remaining valid even if the identity provider ceases to exist. To achieve this, individuals must be able to self assert attributes as well as receive attributes potentially from multiple sources, so that if one ceases to exist, other attributes remain valid. Still a user have the rigth to remove some of the operational data from the SSI system. | The definition biases on the statement of [^sheldrake][^stokkink] that claim identity should exist forever, or at least the user wish. Therefore, we disagreed with the definition of [^ferdous] that claims that the identity ceases to exist if the asserted authority ceases. Hence, the original set of definitions was then clarified stating how this will be possible. The survey harden this statement. |
| Portability | An identity and its associated data should be easily transportable from one platform to another platform, without legal, political, or technological restrictions. | A very important property for a large majority of respondents to the survey, even though the definition does not mention the concept of control as stated by [^pava][^satybaldy][^allen]. | 
...
| Cost | Users must be able to performe crucial operation without cost barriers. Mechanisms for creating, maintaining, and sustaining the system should have minimal or no costs. The cost for an identity system must outweight the benefits. | The term refers costs for users and for the entities that join the ecosystem. Its importance reflects the answers from the survey. | 
| Usability ~~and Consistency~~ | An SSI ecosystem shall maximize usability of agents through consistent user experience and interfaces that should allow entities to control, manage, and use their identities intuitively, reliably, and effectively. It must hide underlying complexity, and should be easy to use. | Despites the definitions from authors discuss the recovery from data loss [^andrieu][^naik] and accessiblity[^sovrin], this definition is consistent with the usability of agents and users interfaces. | 
| ~~Decentralization and~~ Autonomy | Users should have autonomy over their identity data without relying on third parties. They must be the only one being resposible for all operations about their identity. | Autonomy is summarized by the definition of authors. | 
| Verifiability ~~and Authenticity~~ | Relying parties should be able to reliably verify users' identities withouh any interaction with the issuers. Relying parties should receive objective evidence that presented digital identities truthfully characterize their owners. | As verification is the new way to authenticate users, we used the term verifiability. |

Finally, we position these principles within the same process flow of Figure 1. We used our new definition of principles.
