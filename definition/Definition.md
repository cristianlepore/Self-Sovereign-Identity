...
The questionnaire consisted of two part. The first five questions aimed to familiarize participants with the candidates' knowledge of identity, while the remaining fifteen questions were designed to validate the SSI properties. While a similar contributions from [^cucko] in the field of SSI, required respondents to go trhgouh a reviewing process of SSI pricniples definition, which lead to the 23.7% of respondent to droput the questionnaire before finishing; we recognized that the time spent completing the questionnaire was an active feature of the survey and could affect the overall response rate. For this reason, to minimize the time-to-completition, we aimed to shorten the questionnaire's completion time by creating a single question for each principle and embedding a brief definition of the principle directly within the question, thus achieving respondent awareness of pricniples throughout the process. Of course, we recognize the necessity to summarize the definition within the question in the proper way, while keeping questions easy to comprehend for a wide audience in the field of identity.
