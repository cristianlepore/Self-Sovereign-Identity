...
The same table is reported in Figure 1. The chart is a dot matrix plot that visualizes the distribution of principles between authors and is instrumetal to visualize future relationships. Indeed, we will use the same chart for the ret of our exercise. The y-axis lists different authors, while the x-axis lists the principles from authors, covering aspects such as transparency, privacy, security, autonomy, and decentralization. Each blue dot signifies an instance where an author has referenced or included a particular principle in their work. A first peek shows that some authors cited a broad spectrum of principles, while others focus on a select few. Certain principles, such as transparency, privacy, security, and interoperability, appear frequently, indicating their fundamental importance. In contrast, other principles have fewer references, suggesting they are either emerging concepts or less widely recognized. Clusters of dots for specific principles across multiple authors highlight areas of consensus, while sparsely populated areas suggest differences in perspectives. Overall, the chart provides a structured overview of how different principles are adopted across multiple studies or frameworks, showing both commonalities and variations in their recognition and application.

The chart is a dot matrix plot that visually represents the relationship between various authors and principles, offering insights into the frequency and distribution of specific principles across different sources. The y-axis lists authors, likely representing researchers, organizations, or institutions that have contributed to discussions or frameworks related to these principles. The x-axis enumerates a broad set of principles, which appear to be related to digital identity, self-sovereign identity (SSI), data management, or security frameworks. These principles cover fundamental aspects such as **existence, transparency, privacy, security, interoperability, autonomy, decentralization, accountability, compliance, trust, and equity**. Each blue dot signifies an instance where a particular principle has been cited, endorsed, or discussed by an author.
...
REMOVE NOISE; REMOVE DUPLICATE AND REDUNDANT ENTRIES
...
This approach aligns our list with established terminology, and ensures a coherent naming across works. Additionally, we slightly adjusted the work of [^cucko] to reflect updates in a more recent literature review. Specifically, the properties of Equity and Inclusion, and Recoverability as been removed; Legacy System renamed with Compliance; links between Control and Agency, Access and Usability, and Availability and Recoverability have been dismissed. For sake of clarity, the diagram includes only the properties subject to modification or merging and not the all exihibit principles.
...
which calculates the entropy as a measure of the degree of uncertainty of a principle, based on the authors' citations. A high entropy corresponds to a principle cited by manies, while a low value corresponds to a few citations. The entropy does not reflect the frequency, rather is a measeure of how messy is the distribution of probability. So, for a principles that is "predictable", namely frequently cited, the entropy will be low; In the same way, for principles that are almost always false, the entropy will be low as well. The reason is that their degree of predictability is the same. This is visible for principles like "Ownership and Control" and "Simple (see the [Appendix A](#appendix-a-entropy))."
